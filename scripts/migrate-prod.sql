-- =====================================================
-- 실 DB 마이그레이션
-- 적용 순서: 1) 스키마 변경 → 2) 초기 데이터
-- =====================================================

-- 1. scrim_ratings 테이블 생성
...
-- 2. scrim_participants 누락 컬럼 추가
ALTER TABLE scrim_participants ADD COLUMN IF NOT EXISTS cs INT NOT NULL DEFAULT 0;
ALTER TABLE scrim_participants ADD COLUMN IF NOT EXISTS is_mvp TINYINT NOT NULL DEFAULT 0;

-- 3. 내전 MMR + 주라인/부라인 초기값 (init-scrim-data.sql 내용 포함)
...
UPDATE members SET main_line='JG',  sub_line='ADC'  WHERE nickname='골드정도하는사람';
...
UPDATE members SET main_line='MID'                  WHERE nickname='한율꽃';
UPDATE members SET main_line='JG',  sub_line='TOP'  WHERE nickname='JO CHAN YANG';
